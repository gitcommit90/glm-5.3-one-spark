...
CODE_PROMPT = (
    "Write a complete Python class LRUCache with get(key) and put(key, value) methods "
    "using an OrderedDict, with docstrings and type hints, then a short usage example "
    "and a set of pytest unit tests. Output only code."
)
...
def spec_delta(before: dict[str, float], after: dict[str, float]) -> dict:
    ...
    pos = []
    for i in range(int(__import__("os").environ.get("BENCH_K","7"))):
        k = f"pos:{i}"
...
def main() -> int:
    ...
    if h_code != 200:
        ...
        return 2
    prompt = STRUCTURED_PROMPT if args.structured else (CODE_PROMPT if args.kind=="code" else BENCH_PROMPT)
    rec["prompt"] = "structured-count-1-200" if args.structured else ("lrucache-code" if args.kind=="code" else "hashmap-prose")
    rec["thinking"] = False
    rec["temperature"] = 0
    if args.structured:
